...
func main() {
	...
	for scanner.Scan() {
		...
		case *osm.Way:
			if e.Tags.Find("railway") == "rail" {
				...
				// 属性文字のエスケープ関連文字の訂正
				if strings.Contains(e.Tags.Find("operator"), "\\") {
					file.WriteString(fmt.Sprintf(",\"properties\":{\"id\":%d,\"operator\":\"%s\"}}", e.ID, strings.Replace(e.Tags.Find("operator"), "\\", "", -1)))
				} else if strings.Contains(e.Tags.Find("operator"), "\n") {
					file.WriteString(fmt.Sprintf(",\"properties\":{\"id\":%d,\"operator\":\"%s\"}}", e.ID, strings.Replace(e.Tags.Find("operator"), "\n", "", -1)))
				} else if strings.Contains(e.Tags.Find("operator"), "\"") {
					file.WriteString(fmt.Sprintf(",\"properties\":{\"id\":%d,\"operator\":\"%s\"}}", e.ID, strings.Replace(e.Tags.Find("operator"), "\"", "　", -1)))
				} else {
					file.WriteString(fmt.Sprintf(",\"properties\":{\"id\":%d,\"operator\":\"%s\"}}", e.ID, e.Tags.Find("operator")))
				}
				targetways++
			}
			ways++
	...
	fmt.Println("targetways:", targetways)
